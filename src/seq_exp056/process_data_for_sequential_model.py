...
def make_features(df: pd.DataFrame):
    ...
    with trace.timer("process num features"):
        ...
        nas = df[na_features].isna().add_suffix("-isna")
        cat_features.extend([f"{col}-isna" for col in na_features])

        ## concat
        df = pd.concat([df, nas], axis=1)
        del nas
        gc.collect()
...
def scale_features(df, num_features, cat_features, type="train"):
    ...
    with trace.timer("clip outliers"):
        ...
        for col in num_features:
            df[col] = df[col].astype(np.float32).clip(lowers[col], uppers[col])
...
def prepare_data(TYPE):
    def reshape_pad(x: pd.DataFrame):
        ...
        ## interpolating
        # return np.pad(
        #     x.interpolate(limit_direction="both", axis=0).fillna(0).astype(np.float32).to_numpy(),
        #     ((13 - l, 0), (0, 0)),
        #     constant_values=0,
        # )
