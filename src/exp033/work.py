...
def aggregate_features(df, filename):
    ...
    with trace.timer("aggregate num features"):
        ...
        # diff_smooth
        last_diff1 = df.groupby("customer_ID")[num_columns].nth(-1).fillna(0).abs() - df.groupby("customer_ID")[num_columns].nth(-2).fillna(0).abs()
        last_diff2 = df.groupby("customer_ID")[num_columns].nth(-2).fillna(0).abs() - df.groupby("customer_ID")[num_columns].nth(-3).fillna(0).abs()
        last_diff3 = df.groupby("customer_ID")[num_columns].nth(-3).fillna(0).abs() - df.groupby("customer_ID")[num_columns].nth(-4).fillna(0).abs()
        last_diff4 = df.groupby("customer_ID")[num_columns].nth(-4).fillna(0).abs() - df.groupby("customer_ID")[num_columns].nth(-5).fillna(0).abs()
        last_diff5 = df.groupby("customer_ID")[num_columns].nth(-5).fillna(0).abs() - df.groupby("customer_ID")[num_columns].nth(-6).fillna(0).abs()
        last_diff_smooth3 = ((last_diff1 + last_diff2 + last_diff3) / 3).add_suffix("-last_diff_smooth3")
        last_diff_smooth5 = ((last_diff1 + last_diff2 + last_diff3 + last_diff4 + last_diff5) / 5).add_suffix("-last_diff_smooth5")
        results.append(last_diff_smooth3.sort_index())
        results.append(last_diff_smooth5.sort_index())
